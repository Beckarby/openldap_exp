...
```bash
ldapwhoami -x -H ldap://localhost -D "uid=andres.garcia,ou=users,dc=atlas,dc=com" -w jojos
```
...
```bash
ldapadd -x -H ldap://localhost -D "uid=andres.garcia,ou=users,dc=atlas,dc=com" -w jojos <<EOF
...
```bash
ldapdelete -x -H ldap://localhost -D "uid=andres.garcia,ou=users,dc=atlas,dc=com" -w jojos "cn=admins,ou=groups,dc=atlas,dc=com"
```
...
### 5. Password change 
```bash
ldappasswd -x -H ldap://localhost -D "uid=andres.garcia,ou=users,dc=atlas,dc=com" -w jojos -s newpassword
```
